...
var game_labelPointList = function() {
		...
		var aoColumns = [
				...
				{		
		            ...
		            "fnCreatedCell" : function(nTd, sData, oData, iRow, iCol) {
		            	if(oData.dealerType=="1"){
		            		$(nTd).append("企业");
		            	}else if(oData.dealerType=="2"){
		            		$(nTd).append("私人");
		            	}
		            	else{
		            	    $(nTd).append("暂无");
				...
				
				{
					...
					"sWidth" : "40%",
						...
						"sVisible" : false,
						"fnCreatedCell" : function(nTd,sData, oData, iRow, iCol) {
								$(nTd).append(changeDateFormat(oData.lastUpdateTime));
						}
				}];
							...
							$(nTd)
									.append(
											"<span style='margin: 0 5px 0 5px'><span class='btn btn-xs purple ajaxify'"
													+ " href='updatechannel.html?tagsId="
													+ oData.id
	...
	var loadTableData = function() {
		...
		if(selectWarehouseName){
			reqUrl="queryChanneldealerListByPage?dealerName="+selectWarehouseName;
		}else{
			reqUrl="queryChanneldealerListByPage";
		}
