var Orderform_order = function () {
    ...
    var initTableHeaderInfo = function(){
        var aoColumns = [
			...
			{ "sTitle": "退货方式",'sWidth':'4%', "mData": "ext4" },
			{
				"sTitle": "退货原因",
				"mDataProp" : "",
				"sDefaultContent" : "",
    ...
    var loadTableData = function(){
        ...
        oTable = $('#'+tableId).DataTable({
            ...
            "sAjaxSource": "queryOrderRuturnByPage",
      ...
      if (jQuery().timepicker) {
        ...
        $("#confirmBut").on("click", function(){
            ...
            $.ajax({
                type : "post",
                dataType : "json",
                url : "orderReturn/finishOrderReturnById",
                data : param,
                success : function(data) {
	    ...
	    $('#'+tableId).on('click', ' tbody td .row-details', function () {
		    ...
		    $("#exportExcel").on("click", function(){
		    	...
		    	if(confirm("订单导出时间有点久，确定要执行此操作？")){	
		    		var params = 'outNo='+outNo+'&telPhone='+telPhone+'&status='+status;
			    	var url = '/order/exportReturnOrderlistExcel';
