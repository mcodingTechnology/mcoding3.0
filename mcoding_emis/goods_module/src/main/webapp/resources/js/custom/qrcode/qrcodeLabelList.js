...
var qrcode_qrcodeLabel = function () {
    ...
    var initTableHeaderInfo = function(){
        var aoColumns = [
			...
			{ "sTitle": "键值","sWidth" : "1%", "mData": "qrcodekey"},
			{ "sTitle": "名称", "sWidth" : "8%", "mData": "qrcodename"},
			{ "sTitle": "临时二维码图片",
				"mDataProp" : "",
				"sDefaultContent" : "",
            	"sWidth" : "5%",
                ...
                "sVisible" : false,
                "fnCreatedCell" : function(nTd,sData, oData, iRow, iCol) {
                    $(nTd).append("<img src='"+oData.perimgurl+"'  style='width:40px;height:40px' onMouseOver='showimg(this)'>");
            ...
            aoColumns.push({
            	...
            	"fnCreatedCell": function(nTd,sData, oData, iRow, iCol){
            		...
            		if(App.checkUserOperRight("qrcodeLabelList", "downloadTmpQrcode")){
            			$(nTd).append("<span style='margin: 0 5px 0 5px'><span class='btn btn-xs purple ajaxify'" +
            					" href='WechatAPI/downloadTmpQrcode?sceneId="+oData.id+"&&expire_seconds="+oData.ext+"'><i class='fa fa-edit'></i> 生成临时二维码 </span>");
            		}
                    $(nTd).append("<span style='margin: 0 5px 0 5px'><span class='btn btn-xs purple ajaxify'" +
                            " href='WechatAPI/downloadPerQrcode?sceneId="+oData.id+"'></i> 生成永久二维码 </span>");
    ...
    var loadTableData = function(){
        ...
        oTable = $('#'+tableId).DataTable({
            ...
            "sAjaxSource": "queryQRCodeLabelData",
...
function downloadTmpQrcode(){
	$.ajax({
        type: "POST",
        url: "WechatAPI/downloadTmpQrcode",
        data: {sceneId:5},
        dataType: "json",
        success: function (data) {
            if(data.code==0){
