        ...
        var aoColumns = [
			{
				...
				"fnCreatedCell": function(nTd,sData, oData, iRow, iCol){
					if(App.checkUserOperRight("productList", "editProduct")){
						$(nTd).append("<p style='margin: 0 5px 10px 0px'><span class='btn btn-xs purple ajaxify' href='addProductView?productId="+oData.productId+"'><i class='fa fa-edit'></i> 编辑 </span></p>");
						...
						$(nTd).append("<span style='margin: 0 5px 10px 0px'><span class='btn btn-xs purple ajaxify' href='addProductView?productId="+oData.productId+"'  disabled='disabled'><i class='fa fa-edit'></i> 编辑 </span><br/>");
						//                		$(nTd).append("<span style='margin: 0 5px 0 5px'><span class='btn purple btn-xs black' data-toggle='modal' href='#confirmWin2' id="+oData.productId+"><i class='fa fa-edit'></i> 加盟商提成 </span>");
					}
					if(App.checkUserOperRight("productList", "deleteProduct")){
						$(nTd).append("<span class='btn default btn-xs black' data-toggle='modal' href='#confirmWin' id="+oData.productId+"><i class='fa fa-trash-o'></i> 删除 </span>");
					}else{
						$(nTd).append("<span><span class='btn default btn-xs black' data-toggle='modal' href='#confirmWin' id="+oData.productId+"  disabled='disabled'><i class='fa fa-trash-o'></i> 删除 </span>");
					}
				}
			}
        ...
        var aoColumnDefs =[];
            aoColumns.push(
...

				{
					...
					"sVisible" : false,
					"fnCreatedCell" : function(nTd, sData, oData, iRow, iCol) {
							$(nTd).append('<div style="white-space: normal;word-break:break-all;width:100px;">'+oData.productName+'</div>');
					}
				},
				{
					"sTitle": "原价",
				...
				},
				{
					...
					"fnCreatedCell" : function(nTd, sData, oData, iRow, iCol) {
						...
						if(App.checkUserOperRight("productList", "productSort")){
				...
				},
				{
					...
					"sVisible" : false,
					"fnCreatedCell" : function(nTd, sData, oData, iRow, iCol) {
						$(nTd).append("<label class='label label-success'>"+oData.productLabel+"</label>");
					}
				},
				{
					"sTitle": "官网展示",
					...
					"sVisible" : false,
					"fnCreatedCell" : function(nTd, sData, oData, iRow, iCol) {
						if(oData.isShowInOfficialWeb==1){
	...
	$.ajax({
		...
		success: function (data) {
			if(data.code=="0"){
				...
				alert("修改失败，排序重复");
				$("#productSequenceInput"+productId).val(data.data);
			}
		}               
	});
}
