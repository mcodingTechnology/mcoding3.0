...
var RuleEngine_AddRuleEngineConfManager = function() {
	...
	var initEvent = function() {
		...
		$("#singleAdd").on("click", function() {
			...
			$("#origPrice").val($("#origPrice").val()*100);
			$("#promoMinLimit").val($("#promoMinLimit").val()*100);
			$("#returnBack").val($("#returnBack").val()*100);
			$("#promoPrice").val($("#promoPrice").val()*100);
			$("#freight").val($("#freight").val()*100);
			
			var requestURL = "saveRuleEngineConfig";
			var tips = "增加失败!";
			if ($("#id").val() != '') {
				requestURL = "saveRuleEngineConfig";
				tips = "修改失败!";
			}
		...
		var options = {
			...
			rules : {
				...
				restrictionNum : {
					validRestrictionNum : true
				},
				stockRestrictionNum : {
					validStockRestrictionNum : true
				}
		...
		jQuery.validator.addMethod("validReturnBack2", function(value, element) {
			var ruleType = $("#ruleType").val();
			if(ruleType == 1){
				var returnBackVal = $("#returnBack").val();
				var promoMinLimitVal = $("#promoMinLimit").val()
				if(!returnBackVal){
					returnBackVal = 0;
				}
				if(!promoMinLimitVal){
				...
				if(value == "" || returnBackVal >= promoMinLimitVal){
					return false;
				}else {
					return true;
				}
		...
		jQuery.validator.addMethod("validRestrictionNum", function(value, element) {
			var ruleType = $("#ruleType").val();
			var reg = positiveIntegerReg;
			if(ruleType == 5){
				if(value == ""){
		...
		jQuery.validator.addMethod("validStockRestrictionNum", function(value, element) {
			var ruleType = $("#ruleType").val();
			var reg = positiveIntegerReg;
			if(ruleType == 5){
				if(value == ""){
...
//            	$("#brandCodeDiv").show();
            	...
            	$("#restrictionNumDiv").show();
            	$("#stockRestrictionNumDiv").show();
            }else if(type == 6){
            	$("#productIdDiv").show();
            	$("#origPriceDiv").show();
            	$("#startEndTimeDiv").show();
...
//            	$("#brandCodeDiv").show();
        ...
        $("#productId").on("change", function(){
            ...
            $.ajax({
        		type: "post",
        		url:  "getProductPriceByProductId?productId="+productId,
        		async: true,
        		global: false,
        		dataType: "json",
        		contentType: "application/json; charset=utf-8",
        		success: function(data) {
        			$("#origPrice").val(data/100);
        		}
        	});
        ...
        $("#brandCode").on("change", function() {
    		...
    		$.ajax({
    			type : "POST",
    			url : "getProductAndGiftListByBrandCode?brandCode=" + $(this).val(),
    			dataType : "json",
    			success : function(data) {
    				if (data.code == 0) {
...
//		hideDivClearText("brandCodeDiv");
		...
		hideDivClearText("restrictionNumDiv");
		hideDivClearText("stockRestrictionNumDiv");
	}
...
//		hideDiv("brandCodeDiv");
		...
		hideDiv("restrictionNumDiv");
		hideDiv("stockRestrictionNumDiv");
	}
