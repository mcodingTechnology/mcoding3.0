			...
			{ 
				...
				"fnCreatedCell" : function(nTd,sData, oData, iRow, iCol) {
					if(oData.brandCode=="MRMJ"){
						$(nTd).append("<label class='label label-success'>极智购</label>");
					}else{
						$(nTd).append("<label class='label label-info'>暂无</label>");
					}
			...
			{ 
				...
				"fnCreatedCell" : function(nTd,sData, oData, iRow, iCol) {
					if(oData.channelsId=="1"){
						$(nTd).append("<label class='label label-success'>战略合伙人</label>");
					}else if (oData.channelsId=="2"){
						$(nTd).append("<label class='label label-info'>特级合伙人</label>");
					}else if (oData.channelsId=="3"){
						$(nTd).append("<label class='label label-info'>一级合伙人</label>");
					}else if (oData.channelsId=="4"){
						$(nTd).append("<label class='label label-info'>二级合伙人</label>");
					}else if (oData.channelsId=="5"){
						$(nTd).append("<label class='label label-info'>会员</label>");
					}
        ...
        var aoColumnDefs =[];
            aoColumns.push({
            	...
            	"fnCreatedCell": function(nTd,sData, oData, iRow, iCol){
					...
					if(App.checkUserOperRight("memberList", "addMemberPoint")){
						$(nTd).append("<span style='margin: 0 5px 0 5px'><span class='btn btn-xs purple ajaxify' " +
							"href='addMemberPointPageView.html?memberName="+oData.fullName+"&memberId="+oData.memberId+"&mobilePhone="+oData.mobilePhone+"'><i class='fa fa-edit'></i> 手动会员积分 </span>");
					}
					$(nTd).append("<span class='btn btn-xs purple black' data-toggle='modal' " +
						">" +
						"<a style='color:white;text-decoration:none' target='_blank' href='http://v.gymmaxer.com/api/wechatTestGMX.html?openid="+oData.openid+"'>模拟登录</a> </span>&nbsp;&nbsp;");
					if($('#memberTable_length').html().indexOf("总会员人数") == -1){
						$('#memberTable_length').append("<label style='margin-left: 10px;'>总会员人数:"+oData.memberCount+"人</label>");
					}
					if(App.checkUserOperRight("memberList", "addMemberPoint")){
						$(nTd).append("<span style='btn btn-xs purple black'><span class='btn btn-xs purple ajaxify' " +
							"href='userTagsList.html?memberName="+oData.fullName+"&memberId="+oData.memberId+"'><i class='fa fa-edit'></i> 贴标签 </span>");
					}
					$(nTd).append("<span style='margin: 0 5px 0 5px'><span class='btn btn-xs purple ajaxify' href='merriplusApi/editMemberLevelView.html?memberId="+oData.memberId+"'><i class='fa fa-edit'></i> 修改等级 </span>");
                }
            });
      ...
      //触发相关赋值
        $("#confirmWin2").on("show.bs.modal", function(e){
            ...
            $.ajax({
                type : "post",
                dataType : "json",
                url : "merriplusApi/getMemberDetail",
                data : param,
                success : function(data) {
                    if(data.status=='00'){
