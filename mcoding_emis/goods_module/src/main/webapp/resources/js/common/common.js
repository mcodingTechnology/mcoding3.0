...
var buiPlug = {
	loading: {
		set: function(text) {
			...
			if($('[bui-loading]').length <= 0) {
				$("body").append('<div bui-loading style="display:none;"></div>');
				$("[bui-loading]").append('' +
					'<i style="width:0;height:100%;display:inline-block;vertical-align:middle"></i>' +
					'<div style="width:90%;display:inline-block;vertical-align:middle"><i class="fa fa-spinner fa-5x fa-spin"></i><p style="font-size:14px;margin-top:6px;">' + text + '</p ></div>' +
					'<i style="width:0;height:100%;display:inline-block;vertical-align:middle;"></i>').css({
					'color': 'white',
					'position': 'fixed',
					'width': '100%',
					'height': '100%',
