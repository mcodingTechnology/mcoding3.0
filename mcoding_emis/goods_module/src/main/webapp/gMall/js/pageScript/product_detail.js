...
$(document).ready(function() {
	...
	setTimeout(function() {
		//获取产品信息
		global_getJsonSetVue(workSpace.jsonUrl + '/merriplusApi/getProductById', 'productDetail', {
			...
			setTimeout(function() {
				...
				global_wechatGetConfig({
					...
					success: function(data) {
						...
						golbal_wechatShare({
							jsonUrl: workSpace.jsonUrl, //接口路径
							title: vueData.memberDetail.data.fullName + '给你分享了' + vueData.productDetail.data.productName, // 分享标题
							link: window.location.href, // 分享链接
							desc: '快点来看看吧...', // 分享描述
							imgUrl: vueData.productDetail.data.microproductrollimg1, // 分享图标
							cancel: function() {
								buijs_alert({
									content: '取消分享'
								})
				...
				$("#productDetailSwiper").height($("#productDetailSwiper").width() * 1 / 1).buijs_swiper({
					setid: 'productDetailSwiper',
					btn: false,
				});
				$("#productDetailSwiperMask").remove();
				//tab
...
function productDetail_buy() {
	...
	buijs_modal({
		...
		trueAfter: function() {
			...
			setTimeout(function() {
				window.location.href = 'order_add.html?type=single&productNum=' + vueData.buyNum + '&productId=' + vueData.productDetail.data.productId + '&productLabel=' + vueData.urlData.productLabel
			}, 1000)
		}
	});
};
...
function productDetail_createPic() {
	if(!vueData.contactWithQrcode.data) {
		buijs_alert({
		...
		buijs_modal({
			...
			footer: '<div class="bui_p_24 bui_ts_12 bui_ta_c">请长按保存海报并发给您的朋友，就可以赚钱了</div>',
			footerClass: '',
