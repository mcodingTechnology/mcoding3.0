...
vueObj.brandNameEn = 'Gymmaxer'; //项目英文名称
vueObj.brandCode = 'JLD'; //公用brandCode
vueObj.mallType = 'gMall'; //公用mallType
vueObj.mallPath = 'gMall/'; //目录名称
vueObj.shareLogo = 'http://selangsela.com/EMIS/gMall/pic/share_logo.jpg'; //分享图片路径
...
vueObj.giftMallPath = '../GiftMall_GMX/';
vueObj.signPath = 'sign.html?gameid=33';
vueObj.memberInfo = {};
...
$(document).ready(function() {
	...
	//重写title
	$("title").html(vueObj.brandName + ' 移动商城 ' + vueObj.brandNameEn + ' Mall');
	...
	if(vueObj.isShare == true) {
		...
		if(!hasWx) {
			$.ajax({
				...
				error: function() {},
				success: function() {
					wechatSharePublic("我正在逛" + vueObj.brandName + "微商城", "福利这种事，脸皮厚抢个够，脸皮薄拿不着！", "", false, "", vueObj.shareLogo);
				}
			});
		};
			...
			success: function(data) {
				if(!data.data) {
					...
					buijs_modal({
						...
						footer: '<hr><div class="bui_p_12"><button class="bui_btn_48 bui_block ' + vueObj.style.btnTrue + '" onclick="window.location.reload();">重新登录</button></div>',
						showAfter: function() {
							$("#checkReload").css('z-index', 100000);
						}
					});
				} else {
...
function template_prodcutlist(options) {
	...
	return '<li>' +
		'<a href="' + setting.linkUrl + '"  class="bui_p_12 bui_ta_c bui_block bui_bds_1_b bui_bds_1_r">' +
