var buijs_getFile = {
	css: {
		bicon: '',
		fontAwesome: 'http://cdn.bootcss.com/font-awesome/4.6.3/css/font-awesome.min.css',
		highlight: 'http://cdn.bootcss.com/highlight.js/9.6.0/styles/zenburn.min.css'
	},
	js: {
		lazyload: 'http://cdn.bootcss.com/jquery_lazyload/1.9.7/jquery.lazyload.min.js',
		swiper: 'https://cdn.bootcss.com/Swiper/3.3.1/js/swiper.jquery.min.js',
		dataTimePicker: 'http://www.baidiui.com/baidiui_v2.0.3/js/baidiui-datetimepicker.min.js',
		highlight: 'http://cdn.bootcss.com/highlight.js/9.6.0/highlight.min.js',
		vue: 'http://cdn.bootcss.com/vue/2.0.3/vue.min.js'
	}
};
...
//动态加载JS,CSS
function buijs_loadFile(fileType, fileUrl, callback) {
	var _header = $('head:first');
	//css
	if(fileType == 'css') {
		if($("head link[href='" + fileUrl + "']").length == 0) {
	...
	//js
		...
		if($("head script[src='" + fileUrl + "']").length == 0) {
...
var buijs_modal = function(options) {
	var buijs_modal = {
		...
		content: function(setObj) {
			...
			if($("#" + setObj.setid).length == 0) {
				$('.bui_modal_mask').before(
					'<div id="' + setObj.setid + '" class="bui_modal ' + setObj.effect + ' ' + setObj.positions + ' ' + setObj.boxClass + '" style="width:' + setObj.width + ';height:' + setObj.height + ';z-index:' + zindex + ';display:none;">' +
				...
				$('#' + setObj.setid).show().insertAfter('.bui_modal_mask').addClass('bui_modal ' + setObj.effect + ' ' + setObj.positions + ' ' + setObj.boxClass).css({
					'width': setObj.width,
					'height': setObj.height,
					'z-index': zindex
				});
				if($('#' + setObj.setid).children('.bui_modal_b').length == 0) {
					$('#' + setObj.setid).wrapInner('<div class="bui_modal_b" style="overflow-x:hidden;overflow-y:auto;"></div>');
				};
				if($('#' + setObj.setid).children('.bui_modal_h').length == 0) {
					$('#' + setObj.setid).prepend('<div class="bui_modal_h ' + setObj.headerClass + '">' + headerHtml + '</div>');
				};
				if($('#' + setObj.setid).children('.bui_modal_f').length == 0) {
					$('#' + setObj.setid).append('<div class="bui_modal_f ' + setObj.footerClass + '">' + footerHtml + '</div>')
				};
			}

			//开启
			setTimeout(function() {
				$("#" + setObj.setid).show().addClass('active');
				...
				if(setObj.ajaxload) {
					setTimeout(function() {
						$("#" + setObj.setid).attr('bui_modal_url', setObj.ajaxload).find('.bui_modal_b').load(setObj.ajaxload);
					}, 600);
				};
				return false;
			}, 100);
	...
	var defaults = {
		...
		effect: 'zoomOut', //动画效果
		title: '提示', //默认标题
		content: '<div class="bui_p_24 bui_ta_c bui_ts_24_i"><i class="fa fa-circle-o-notch fa-spin bui_tc_white_d48_i"></i></div>', //默认内容
...
function buijs_modal_close(id) {
	...
	setTimeout(function() {
		closeBox.map(function() {
			if($(this).attr('buijs_modal_isremove') == 'false') {
				$(this).hide();
			} else {
				$(this).remove();
			}
		});
		if($(".bui_modal.active").length <= 0) {
	...
	$(this).map(function(index, data) {
		...
		buijs_loadFile('js', buijs_getFile.js.swiper, function() {
			new Swiper(_t, {
				...
				slideToClickedSlide: false, //开启点击item图片跳转
				autoHeight: true,
				...
				onInit: function(swiper) {
					...
					setObj.onInit ? setObj.onInit() : null;
				},
				//滚动前
...
$.fn.buijs_date = function(options) {
	...
	buijs_loadFile('js', buijs_getFile.js.dataTimePicker, function() {
		_t.map(function() {
			...
			setObj.showAfter = labelData.showAfter || setting.showAfter;
			setObj.hideAfter = labelData.hideAfter || setting.hideAfter;
			setObj.changeAfter = labelData.changeAfter || setting.changeAfter;
			//开启插件
			$(this).datetimepicker({
...
$.fn.buijs_form_check = function(options) {
	$(this).map(function() {
		...
		var _check = function(_t) {
			var _input = _t.find('input,select,textarea');
			...
			if(_t.attr('buijs_form_item_type').indexOf('mobile') != -1) {
				...
				var rule = /1[3-8]+\d{9}/;
				var _tipSuccess = setObj.textsuccess || _t.attr('buijs_form_item_success') || 'Mobile Complete',
					_tipError = setObj.texterror || _t.attr('buijs_form_item_error') || 'Mobile invalid';
			...
			if(_t.attr('buijs_form_item_type').indexOf('email') != -1) {
				...
				var _tipSuccess = setObj.textsuccess || _t.attr('buijs_form_item_success') || 'Email Complete',
					_tipError = setObj.texterror || _t.attr('buijs_form_item_error') || 'Email invalid';
			...
			if(_t.attr('buijs_form_item_type').indexOf('url') != -1) {
				...
				var _tipSuccess = setObj.textsuccess || _t.attr('buijs_form_item_success') || 'Url Complete',
					_tipError = setObj.texterror || _t.attr('buijs_form_item_error') || 'Url invalid';
				if(_input.val().search(rule) != -1) {
					_success(_tipSuccess);
				} else {
					_error(_tipError);
			...
			//格式校验-完全匹配
			if(_t.attr('buijs_form_item_type').indexOf('match') != -1) {
				var _tipSuccess = setObj.textsuccess || _t.attr('buijs_form_item_success') || 'Match Complete',
					_tipError = setObj.texterror || _t.attr('buijs_form_item_error') || 'Match invalid';

				if(_input.val() == _t.attr('buijs_form_item_match')) {
					_success(_tipSuccess);
				} else {
					_error(_tipError);
				};
			...
			if(_t.attr('buijs_form_item_type').indexOf('number') != -1 || _t.attr('buijs_form_item_type').indexOf('letter') != -1 || _t.attr('buijs_form_item_type').indexOf('symbol') != -1 || _t.attr('buijs_form_item_type').indexOf('cn') != -1) {
				...
				var _number = '',
					...
					_cn = '';
				var _tipSuccess = '',
					_tipError = setObj.texterror || '';
				//校验数字
				if(_t.attr('buijs_form_item_type').indexOf('number') != -1) {
					_number = '0-9\.\-';
					_tipSuccess = setObj.textsuccess || _t.attr('buijs_form_item_success') || 'Complete';
					_tipError = setObj.texterror || _t.attr('buijs_form_item_error') || 'Only input numbers';
				};
				//校验英文字母
				if(_t.attr('buijs_form_item_type').indexOf('letter') != -1) {
					_letter = 'A-Za-z';
					_tipSuccess = setObj.textsuccess || _t.attr('buijs_form_item_success') || 'Complete';
					_tipError = setObj.texterror || _t.attr('buijs_form_item_error') || 'Only input letters';
				};
				//校验符号
				if(_t.attr('buijs_form_item_type').indexOf('symbol') != -1) {
					_symbol = '\-\_\@';
					_tipSuccess = setObj.textsuccess || _t.attr('buijs_form_item_success') || 'Complete';
					_tipError = setObj.texterror || _t.attr('buijs_form_item_error') || 'Only input symbol';
				};
				//校验中文字母
				if(_t.attr('buijs_form_item_type').indexOf('cn') != -1) {
					_cn = '\u4E00-\uFA29';
					_tipSuccess = setObj.textsuccess || _t.attr('buijs_form_item_success') || 'Complete';
					_tipError = setObj.texterror || _t.attr('buijs_form_item_error') || 'Only input Chinese characters';
				};
			...
			if(_t.attr('buijs_form_item_type').indexOf('length') != -1) {
				...
				var _tipSuccess = setObj.textsuccess || _t.attr('buijs_form_item_success') || 'Complete',
					_tipError = setObj.texterror || _t.attr('buijs_form_item_error') || 'At least ' + max + ' up to ' + min;
			...
			if(_t.attr('buijs_form_item_type').indexOf('select') != -1) {
				var _tipSuccess = setObj.textsuccess || _t.attr('buijs_form_item_success') || 'Complete',
					_tipError = setObj.texterror || _t.attr('buijs_form_item_error') || 'Please choose one of them';
		...
		item.map(function() {
			...
			if(_type == 'checkbox') {
				...
				_t.bind('select', function() {
					_check(_t);
				});
			} else {
				_t.bind('input propertychange', function() {
					_check(_t);
				});
			}
...
//页面摇一摇
var buijs_shake_isRuning = false;

function buijs_shake(options) {
	var defaults = {
		shakeThresHold: 3000,
		noSupport: function() {
			buijs_alert({
				content: '您的手机不支持摇一摇功能'
			})
		},
		success: function() {}
	}
	...
	if(window.DeviceMotionEvent) {
		window.addEventListener('devicemotion', buijs_shake_action, false);
	} else {
		setObj.noSupport();
	}

	function buijs_shake_action(eventData) {
		...
		if((curTime - lastUpdate) > 100) {
			...
			if(speed > setObj.shakeThresHold && buijs_shake_isRuning == false) {
				setObj.success();
			}
			last_x = x;
			last_y = y;
...
function buijs_shake_on() {
	buijs_shake_isRuning = false
}

function buijs_shake_off() {
	buijs_shake_isRuning = true
}
...
$.fn.buijs_scrollTobottom = function(callback) {
	var _box = $(this);
	if(_box.children('#buijs_InnerBox').length == 0) {
		_box.wrapInner('<div id="buijs_InnerBox"></div>');
	}
	var _InnerBox = _box.children('#buijs_InnerBox');
	scroll();
	_box.unbind().bind('scroll', function() {
		scroll();
	});

	function scroll() {
		var _scrollB = _box.height() + _box.scrollTop();
		var _innerH = _InnerBox.height();
		if(_scrollB >= _innerH - 6) {
			callback();
		};
	};
};
