...
function getIcon(){
	var param = {
		iconCode:"wMall_index_logo256x256"
	}
	$.ajax({
        type: "POST",
        url: _jsonUrl+"icon/getIconList",
        data: param,
        dataType: "json",
        success: function (data) {
           if(data.queryResult.length > 0){
        	   var img = data.queryResult[0].img;
        	   if(img){
        		   $("#wMallIndexIcon").removeAttr("src");
        		   $("#wMallIndexIcon").attr({ src: img});
        	   }
           }
        }
    });
...
//				};
				_list += template_prodcutlist({
					linkUrl: 'product_content.html?v1.0.0&productId=' + data.productId,
