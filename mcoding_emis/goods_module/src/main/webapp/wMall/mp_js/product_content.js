	...
	$.ajax({
		...
		success: function(pc) {
			...
			//上架
				...
				//限购
				if(isLimit == 1 && vueObj.memberDetail.channelsId == 5) {
					//无库存
							...
							//未买过
								...
								if(limitNum == 1) {
									$('.bui_mo_f').html('<div class="bui_m_8">' +
										'	<li><a href="order.html?v1.0.0&orderType=single&productId=' + vueObj.productDetail.productId + '&productnum=1&productLabel=isMatchingRules" class="bui_btn_48 bui_block bui_tc_white ' + vueObj.style.btnTrue + '">限量抢购 <i class="fa fa-check-square-o fa-fw"></i></a>' +
										'	</li>' +
										'</div>');
								}
								//限购超过一个
					...
					$.ajax({
						...
						success: function() {
							...
							wechatSharePublic("极智构中秋送礼", "免费领取中秋节豪礼", link, false, "", pc.data.productCoverImg);
						}
					});
				}
				//正常
			...
			var minimumQuantity = pc.data.minimumQuantity;
			var priceContent = '';
...
//			} else {
			if (vueObj.memberDetail.channelsId != 5) {
				priceContent = '<p  class="bui_tc_lgray"><span class="bui_tc_black">起订量:</span><span  class="bui_ts_16 ' + vueObj.style.tcTrue + '">' + minimumQuantity + '</span></p>' +
				'<p  class="bui_tc_lgray"><span class="bui_tc_black">单价:</span>￥<span class="mp_price" id="originalPrice">' + (originalPrice || 0) / 100 + '</span>  <span  class="bui_ts_16 ' + vueObj.style.tcTrue + '">￥' + (discountPrice || 0) / 100 + '</span></p>';
			} else {
				...
				onTouch($('.mp_cart_commit'), function(rs, _this) {
					if(rs == 'touch') {
						if(flag == 'pay') {
							...
							window.location.href = 'order.html?v1.0.0&orderType=single&productId=' + productId + '&productnum=' + productnum;
						} else {
							//							if(isLimit==1){
							//								js_msg({
