...
$(document).ready(function() {
	...
	if (productLabel == 'interiorProduct') {
		...
		$.ajax({
			...
			error: function() {},
			success: function() {
				wechatSharePublic("极智构员工内购专场", "全场超低价，下单请备注部门+姓名", _jsonUrl+"/wMall/product_list.html?v1.0.0&productLabel=interiorProduct", false, "", vueObj.shareLogo);
			}
		});
	...
	}else if(productLabel == 'personalInteriorProduct'){
		vueObj.isShare = false;
		$.ajax({
			type: "get",
			url: "../resources/js/common/wechatShare.js",
			...
			error: function() {},
			success: function() {
				wechatSharePublic("极智构超低价员工内购专场", "所有商品全部2折起", _jsonUrl+"/wMall/product_list.html?v1.0.0&productLabel=personalInteriorProduct", false, "", vueObj.shareLogo);
			}
		});
	}
});
