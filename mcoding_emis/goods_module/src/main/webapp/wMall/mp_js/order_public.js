...
function orderReturn(orderid) {
	var returnCompany = '';
	$.map(['请选退货方式', '顾客到超市退换货', '工作人员上门退换货'], function(data) {
	...
	buijs_modal({
		...
		content: '<div class="bui_p_12">' +
			...
			'	</li>' +
			'	<li style="display:none;"><input class="bui_ipt_32 bui_bgc_white bui_block bui_ts_14 bui_radius" maxlength="255" name="returnCode" placeholder="物流单号" value="88888888"/></li>' +
	...
	$("#orderReturnSubmit").unbind().bind('click', function() {
		//前端校验
		if ($("[name=returnCompany] option:selected").val() == '请选退货方式') {
			buijs_alert({
				content: '请选退货方式'
			});
			return false;
