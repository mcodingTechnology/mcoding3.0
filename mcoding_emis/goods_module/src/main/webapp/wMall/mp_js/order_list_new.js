		...
		success: function(data) {
			...
			if (data.queryResult.length != 0) {
				$.map(data.queryResult, function(data) {
					...
					var img = "";
					var productname = "";
					if(data.orderProductsInfo[0] && data.orderProductsInfo[0].productcoverimg){
						img = data.orderProductsInfo[0].productcoverimg;
					}
					if( data.orderProductsInfo[0] &&  data.orderProductsInfo[0].productname){
						productname = data.orderProductsInfo[0].productname;
					}
					_contentBox.append('<li class="bui_panel bui_bgc_white bui_mt_12">' +
						...
						'   <div class="bui_media_l"><img src="' + img + '" style="width:72px;height:72px;"/></div>' +
						'   <div class="bui_media_b">' +
						'     <p class="bui_ts_12 bui_tc_black">' + productname + '</p>' +
						'   </div>' +
