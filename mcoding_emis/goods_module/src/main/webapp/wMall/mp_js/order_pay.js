		...
		success: function(data) {
			...
			if (data.data.orderInfo.paystatus == '待支付') {
				...
				_btn = '<div class="bui_p_8">' +
					'<li><div onclick="javascript:payAction();" class="bui_btn_48 bui_block bui_radius bui_bgc_white bui_bds_0 bui_btn_noline bui_bsd_12" id="payAction"><p class="' + vueObj.style.tcTrue + '">微信支付 <i class="fa fa-angle-right"></i></p></div></li>' +
					'</div>';
				purse_btn = '<div class="bui_p_8">' +
				'<li><div onclick="javascript:pursePay('+data.data.orderInfo.outno+');" class="bui_btn_48 bui_block bui_radius bui_bgc_white bui_bds_0 bui_btn_noline bui_bsd_12" id="pursePay"><p class="' + vueObj.style.tcTrue + '">钱包支付(￥'+myMoney+') <i class="fa fa-angle-right"></i></p></div></li>' +
				'</div>';
				orderNo2 = data.data.orderInfo.outno;
				$("#payBtn2").html(purse_btn);
				
				if((data.data.orderInfo.fee / 100) > myMoney){
					$("#pursePay").attr({ "style": "background-color:#ccc" });
					$("#pursePay").children(".bui_tc_orange").attr({ "style":"color:#999" });
					$("#pursePay").removeAttr("onclick");
				}
			}
			//已支付
			else {
				...
				if (data.data.orderInfo.presentstatus == '待接受') {
					...
					$.ajax({
						...
						error: function() {},
						success: function() {
							wechatSharePublic('送您' + vueObj.brandName + '大礼', '如果不是真的，我马上胖十斤！快来拆礼物！', _jsonUrl + vueObj.mallPath + 'receive_gift.html?orderId=' + _orderId + '&headimgurl=' + vueObj.memberDetail.headimgurl, '', '', vueObj.memberDetail.headimgurl);
						}
					});
				} else {
					_btn = '<div class="bui_p_8">' +
...
function pursePay(){
	if(orderNo2){
		...
		$.ajax({
			type: "post",
			url: _jsonUrl + "/purse/myPursePay?outno="+orderNo2,
			async: false,
			dataType: 'json',
			error: function() {},
			success: function(data) {
