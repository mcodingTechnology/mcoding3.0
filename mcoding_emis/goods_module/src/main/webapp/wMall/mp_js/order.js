...
function order_getOrderData() {
	...
	$.ajax({
		type: "get",
		url: _jsonUrl + "/merriplusApi/handleOrderWithRules?v1.0.0",
		async: false,
		cache: false,
		dataType: 'json',
		data: _jsonData,
...
function order_insetOrderDetail() {
		...
		$.map(orderObj.orderProductList, function(data, index) {
			if(index <= 1) {
				_prodcutListEasy += '<li>' +
					...
					'<p ><span style="font-size: 12px !important;color: #ccc !important;text-decoration: line-through;">￥'+data.originalPrice / 100+'</span><span class="' + vueObj.style.tcTrue + '">￥' + data.price / 100 + '</span></p>' +
					'</div>' +
	...
	$("#priceDetail").html('<li class="bui_bds_1_t" id="orderAmount"><div class="bui_media bui_ptb_12 bui_plr_24">' +
		'	<div class="bui_media_b">' +
		'		<p class="bui_ts_12 bui_tc_white_d96">商品总价</p>' +
		'	</div>' +
		'	<div class="bui_media_r bui_ta_r">' +
		'		<p class="bui_ts_16 ' + vueObj.style.tcTrue + '">￥<span class="priceDetailItem">' + [orderObj.priceDetail.origTotalPrice - orderObj.priceDetail.freight + orderObj.priceDetail.giftPrice] / 100 + '</span><span style="display:none" class="priceDetailItemTotal">'+[orderObj.priceDetail.productTotalPrice + orderObj.priceDetail.payFreight] / 100+'</span></p>' +
		'	</div>' +
		'</div>' +
		'</li>' +
		...
		'</div>' +
		'</li>' +
		(_orderhasParent || vueObj.orderObj.productLabel == 'isMatchingRules' ? '' : '<li class="bui_bds_1_t"><div class="bui_media bui_ptb_12 bui_plr_24">' +
			'	<div class="bui_media_b">' +
...
function orderPriceTotal() {
	var _total = 0;
	$("#priceDetail .priceDetailItemTotal").map(function(index, data) {
		var _t = $(this);
...
function order_getAddress() {
	...
	$.ajax({
		...
		success: function(data) {
			...
			orderObj.productLabel == 'interiorProduct' ? orderObj.addressInfo = '' : orderObj.addressInfo = JSON.parse(_data);
			orderObj.isCompnayAddress = 0; //关闭公司收货，添加运费
			order_getOrderData(); //获取接口_订单数据 by pangxj
			order_insetOrderDetail(); //插入订单内容
			
			var addr=data.data.regson+data.data.address;
			...
			$.ajax({
				type: "GET",
				async: false,
				url: "http://api.map.baidu.com/geocoder/v2/?address="+addr+"&output=json&ak=E7rElbIrn1lVctuW8B5n0dR8r9HlE4Ti&callback=calllocation3",
				//url: "http://apis.map.qq.com/ws/geocoder/v1/?address="+addr+"&key=TA4BZ-BGY3U-XVPVS-BE6B7-YH2C7-NMFRX&output=jsonp&callback=calllocation3",
				dataType: "jsonp",				             
		    	jsonp:'calllocation3'
			});
...
function calllocation3(data){
	console.log(data);
	if(data != undefined && data.status == 0){
		latitude2=data.result.location.lat;
		longitude2=data.result.location.lng;
		console.log("calllocation3 latitude2:"+latitude2);
		console.log("calllocation3 longitude2:"+longitude2);
		
		$.ajax({
			type: "get",
			url: _jsonUrl + "/merriplusApi/routematrix2",
			async: true,
			cache: false,
			data: {"latitude2": latitude2, "longitude2": longitude2},
			dataType: 'json',
			error: function() {
				$("#addressSingle").html('<div class="bui_m_12 bui_p_12 bui_ta_c bui_tc_white_d48"><p><i class="fa fa-unlink bui_fas_48"></i></p><p>位置定位距离计算数据读取失败</p></div>');
			},
			success: function(data) {
				console.log("routematrix2:");
				console.log(data);
				if(data != undefined && data.status == 0){
					//console.log("驾车距离：约"+ data.result.elements[0].distance +"米  耗用时间：约"+ data.result.elements[0].duration + "秒" );
					var distance=parseFloat(data.result[0].distance.value);
					...
					if(distance > 5000.00){
						buijs_modal({
							...
							width: '85%',
							height: '250px',
							content: '<div class="bui_p_32 bui_ta_c"><p>您的收货地址不在配送范围内，</p><p>是否自提？</p></div>',
							footer: '<hr><div class="bui_p_12"><button class="bui_btn_48 ' + vueObj.style.btnTrue + '" style="width:45%;" onclick="commitT();">是</button>&nbsp&nbsp&nbsp<button class="bui_btn_48 ' + vueObj.style.btnTrue + '" style="width:45%;" onclick="commitF();">否</button></div>'
						});
					}
				}else{
					buijs_modal({
...
function commitT(){
	buijs_modal_close();
	$("#commitOrderBtn").css("display","");
}
//否自提隐藏提交订单按钮
function commitF(){
	buijs_modal_close();
	$("#commitOrderBtn").css("display","none");
}
...
function order_companyAddressPanel() {
	...
	buijs_modal({
		...
		width: '85%',
		title: '请选择地址',
		content: orderObj.productLabel == 'interiorProduct' ? companyList : orderObj.productLabel == 'personalInteriorProduct' ? myList : '' // myList + companyList
	})
};
