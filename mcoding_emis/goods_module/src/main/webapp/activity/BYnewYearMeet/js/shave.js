requirejs(['jquery','api','common'],function($,api,common){
	...
	var shake = {
		...
		start:function(time){
			...
			$.post(api.start,{"lottoPrizeGroupId":1,"lottoPrizeGroupStatus":2},function(data){
		...
		stop:function(){
			this.isStart = false;
			//$.post(api.start,{"lottoPrizeGroupId":1,"lottoPrizeGroupStatus":3},function(){
			//},'json')
		},
		getData : function(){
			var _this = this;
			var d = {"lottoPrizeGroupId":1,"fromRow":_this.has}
		...
		add : function(){
			...
			/*var cur = this.arr.pop();
			$('#winners').prepend('');
			$('#winners li').eq(0).animate({
				width: '384px'
			},this.showTime, function() {
				if($('#winners li').length>(shake.showSize+1))  $('#winners li').eq($('#winners li').length-1).remove();
				shake.add();
			});*/
		},
		show:function(cur){
		...
		countDown : function(){
			/*this.t = setInterval(function(){
				...
				if(MM<=0 && SS<=0){
					...
					$('.yTip2').removeClass('animated shake');
					shake.stop();
