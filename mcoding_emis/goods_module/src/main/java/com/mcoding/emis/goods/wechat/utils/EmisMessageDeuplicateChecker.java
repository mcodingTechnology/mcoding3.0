 ...
 *
 */
public class EmisMessageDeuplicateChecker implements WxMessageDuplicateChecker {
	
	private static Logger logger = LoggerFactory.getLogger(EmisMessageDeuplicateChecker.class);
	...
	private ThreadPoolTaskExecutor threadPool = SpringContextHolder.getBean("defaultThreadPool");
	
	public EmisMessageDeuplicateChecker() {
		super();
		startClean();
	}
	...
	protected void cleanExpiredMsgId(){
		logger.debug("start wx msgAndStatus,size["+this.msgAndStatus.size()+"]");
		...
		logger.debug("finish wx msgAndStatus,size["+this.msgAndStatus.size()+"]");
	}
	...
	private void startClean(){
		threadPool.execute(new Runnable() {
			...
			public void run() {
				try {
					while (isVailable) {
						EmisMessageDeuplicateChecker.this.cleanExpiredMsgId();
