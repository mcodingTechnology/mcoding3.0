...
/**
 * BIG生活定时搜索待发货订单，自动发送待发货订单到ERP系统
 * @author hzy
 *
 */
@Service("com.mcoding.emis.goods.schedule.UpdateGMXOrderFromERPJob")
public class UpdateGMXOrderFromERPJob {
	...
	public void execute() throws Exception{
		...
		updateGMXDeliveryInfoByOrder(null,null);
	}

	public void updateGMXDeliveryInfoByOrder(String startDate,String endDate) throws Exception{
		//1.从接口获取前5天的订单，更新物流信息到本地数据库
							...
							for(String oneDealCode : dealCodes){
								...
								if(orderlist.size()>0){
									...
									if(order.getDeliveryorderno()==null){
										...
										//String deliveryName = DeliveryMsg.getDeliverNameByDeliverCode(order.getDeliveryname().trim());//根据配送代码获取配送方式
										try {
											wxMpTemplateMsgUtil.sendWxMpTemplateMessageByType(TemplateMessageRecord.TEMPLATE_MESSAGE_TYPE_GMX_DELIVERY, order.getOpenid(), order.getOutno(),
